...
def convert_to_nunchaku_chroma_single_transformer_block_state_dict(
    state_dict: dict[str, torch.Tensor],
    scale_dict: dict[str, torch.Tensor],
    smooth_dict: dict[str, torch.Tensor],
    branch_dict: dict[str, torch.Tensor],
    ...
    """Convert Chroma single transformer block - similar to FLUX but without norm.linear."""
    down_proj_local_name = "proj_out.linears.1.linear"
    if f"{block_name}.{down_proj_local_name}.weight" not in state_dict:
        down_proj_local_name = "proj_out.linears.1"
        assert f"{block_name}.{down_proj_local_name}.weight" in state_dict

    # Chroma's ChromaAdaLayerNormZeroSinglePruned doesn't have .linear - skip it
    return convert_to_nunchaku_transformer_block_state_dict(
        state_dict=state_dict,
        scale_dict=scale_dict,
        smooth_dict=smooth_dict,
        ...
        block_name=block_name,
        local_name_map={
            # No "norm.linear" for Chroma - it's pruned
            "qkv_proj": ["attn.to_q", "attn.to_k", "attn.to_v"],
            "norm_q": "attn.norm_q",
            "norm_k": "attn.norm_k",
            "out_proj": "proj_out.linears.0",
...
def convert_to_nunchaku_chroma_transformer_block_state_dict(
    state_dict: dict[str, torch.Tensor],
    scale_dict: dict[str, torch.Tensor],
    smooth_dict: dict[str, torch.Tensor],
    branch_dict: dict[str, torch.Tensor],
    ...
    """Convert Chroma joint transformer block - similar to FLUX but without norm1.linear."""
    down_proj_local_name = "ff.net.2.linear"
    if f"{block_name}.{down_proj_local_name}.weight" not in state_dict:
        down_proj_local_name = "ff.net.2"
        assert f"{block_name}.{down_proj_local_name}.weight" in state_dict
    ...
    # Chroma's ChromaAdaLayerNormZeroPruned doesn't have .linear - skip it
    return convert_to_nunchaku_transformer_block_state_dict(
        state_dict=state_dict,
        scale_dict=scale_dict,
        smooth_dict=smooth_dict,
        ...
        block_name=block_name,
        local_name_map={
            # No "norm1.linear" or "norm1_context.linear" for Chroma - they're pruned
            "qkv_proj": ["attn.to_q", "attn.to_k", "attn.to_v"],
            "qkv_proj_context": ["attn.add_q_proj", "attn.add_k_proj", "attn.add_v_proj"],
            "norm_q": "attn.norm_q",
            "norm_k": "attn.norm_k",
        ...
        convert_map={
            # No "norm1.linear" or "norm1_context.linear" entries
            "qkv_proj": "linear",
            "qkv_proj_context": "linear",
            "out_proj": "linear",
            "out_proj_context": "linear",
...
def convert_to_nunchaku_flux_state_dicts(
    ...
    model_type = "Chroma" if is_chroma else "FLUX"
    print(f"Converting {len(block_names)} {model_type} transformer blocks...")
    converted: dict[str, torch.Tensor] = {}
    for block_name in block_names:
        if is_chroma:
            # Use Chroma-specific converters (no norm.linear layers)
            convert_fn = convert_to_nunchaku_chroma_single_transformer_block_state_dict
            if block_name.startswith("transformer_blocks"):
                convert_fn = convert_to_nunchaku_chroma_transformer_block_state_dict
        else:
            # Use FLUX converters
...
if __name__ == "__main__":
    ...
    assert "flux" in model_name.lower() or "chroma" in model_name.lower(), "Only Flux and Chroma models are supported."
    is_chroma = "chroma" in model_name.lower()
