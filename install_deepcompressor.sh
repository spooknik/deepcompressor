...
if grep -q "^diffusers = " pyproject.toml; then
    sed -i 's/^diffusers = .*/diffusers = ">=0.35.0"/' pyproject.toml
    print_success "Set diffusers>=0.35.0"
fi

# Set transformers version (>= 4.48.0 required for peft compatibility)
if grep -q "^transformers = " pyproject.toml; then
    sed -i 's/^transformers = .*/transformers = ">=4.48.0"/' pyproject.toml
    print_success "Set transformers>=4.48.0"
fi

# Set peft version (>= 0.14.0 required for transformers.modeling_layers)
